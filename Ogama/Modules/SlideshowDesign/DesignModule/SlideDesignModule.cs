// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SlideDesignModule.cs" company="Freie Universität Berlin">
...
namespace Ogama.Modules.SlideshowDesign.DesignModule
{
  ...
  public partial class SlideDesignModule : FormWithPicture
  {
    ...
    /// </summary>
    private readonly StimuliTypes stimulusTypeToCreate;
    ...
    public string Description
    {
      set
      {
        this.dltForm.Description = value;
      }
    }
    ...
    /// </summary>
    /// <param name="listBox">
    /// The <see cref="ListBox"/> for which
    ///   the selected items should be deleted.
    /// </param>
    public static void DeleteSelectedItems(ListBox listBox)
    {
      var selectedObjects = listBox.SelectedItems.Cast<object>().ToList();
    ...
    /// </summary>
    /// <param name="combo">
    /// The <see cref="ComboBox"/> to be populated.
    /// </param>
    ...
    /// <summary>
    /// This method updates the position of the selected element in the design picture
      ...
      if (this.designPicture.SelectedElement != null && !this.isInitializingSelectedShape)
      {
        ...
        switch ((DockStyle)this.gveLayoutDockStyle.Value)
        {
          case DockStyle.None:
            break;
          case DockStyle.Top:
            this.designPicture.SelectedElement.Location = new PointF(this.designPicture.SelectedElement.Location.X, 0);
            break;
          case DockStyle.Bottom:
            ...
            break;
          case DockStyle.Left:
            this.designPicture.SelectedElement.Location = new PointF(0, this.designPicture.SelectedElement.Location.Y);
            break;
    ...
    /// An empty <see cref="EventArgs"/>
    /// </param>
    private void SlideDesignModuleLoad(object sender, EventArgs e)
    {
      try
      {
        // Intialize picture
    ...
    /// An empty <see cref="EventArgs"/>
    /// </param>
    private void SlideDesignModuleShown(object sender, EventArgs e)
    {
    ...
    /// </param>
    /// <param name="e">
    /// An <see cref="FormClosingEventArgs"/> that contains the event data.
    /// </param>
    private void FrmStimulusDesignFormClosing(object sender, FormClosingEventArgs e)
    {
      if (this.DialogResult == DialogResult.OK && this.lsbStopConditions.Items.Count == 0)
      {
        e.Cancel = true;
        string message = "You did not specify a response condition in the timing section." + Environment.NewLine
                         + "At least one has to be created, e.g. Time:5000ms" + Environment.NewLine
                         + "Please try again.";
