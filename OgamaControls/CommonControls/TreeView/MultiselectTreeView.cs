  ...
  public class MultiselectTreeView : System.Windows.Forms.TreeView
  {
    ...
    private bool SelectNode(TreeNode tn, bool select, TreeViewAction tva)
    {
      ...
      if (select)
      {
        ...
        // If only one node selected, set tnSelectionMirrorPoint
        if (SelectedNodes.Count == 1)
          tnSelectionMirrorPoint = tn;
    ...
    private delegate void StartEditDelegate();
    ...
    private void StartEdit()
    {
      if (this.InvokeRequired)
      {
        StartEditDelegate startEditDelegate=new StartEditDelegate(StartEdit);
        this.Invoke(startEditDelegate);
        return;
      }
      System.Threading.Thread.Sleep(200);
